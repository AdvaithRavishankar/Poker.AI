...
def _calculate_equity(hand: List[Card], board: List[Card], num_villians: int):
    ...
    while len(villian_hands) < num_villians:
        ...
        while len(villian_hand) < 2:
            ...
            if (
                c not in hand
                and c not in board_copy
                and all(c not in v_hand for v_hand in villian_hands)
            ):
    ...
    return (
        1
        if all(
            evaluate(hand, board_copy) < evaluate(villian_hand, board_copy)
            for villian_hand in villian_hands
        )
        else 0
    )
...
def calculate_equity(
    hand: List[Card], board: List[Card], villian_hands: int, n: int = 25000
):
...
def value_player(game: TexasHoldEm):
    ...
    chips_at_stake = sum(
        game._get_pot(i).get_total_amount() for i in range(player.last_pot + 1)
    )
    pot_odds = game.chips_to_call(player.player_id) / chips_at_stake
    equity = calculate_equity(
        game.get_hand(player.player_id), game.board, len(list(game.active_iter())) - 1
    )
    ...
    if player.state == PlayerState.TO_CALL:
        ...
        if equity + fold_adjuster < pot_odds:
            ...
            if player.chips <= value_bet:
                ...
                raise_amt = (
                    game.player_bet_amount(player.player_id)
                    + game.chips_to_call(player.player_id)
                    + value_bet
                )
                return ActionType.RAISE, min(
                    raise_amt, player.chips + game.player_bet_amount(player.player_id)
                )
        else:
            return ActionType.CALL, None
    else:
